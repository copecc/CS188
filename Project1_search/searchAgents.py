...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                # Remove the corner if it's visited
                nextCorners = tuple(filter(lambda x: x != (nextx, nexty), corners)) 
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if len(remainingCorners) == 0:
        return 0
    # Max distance is both admissible and consistent
    maxDistance = 0
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        if (position, food) not in problem.heuristicInfo: # cache the heuristic value
            problem.heuristicInfo[(position, food)] = mazeDistance(position, food, problem.startingGameState)
        maxDistance = max(maxDistance, problem.heuristicInfo[(position, food)])
    return maxDistance
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        "*** YOUR CODE HERE ***"
        return search.uniformCostSearch(problem) # use UCS to find the closest food
